...
RegisterNetEvent('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        discord = discord and discord:gsub('discord:', '') or 'unknown'

        TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Loaded', 'green', '**' .. GetPlayerName(src) .. '** (<@' .. discord .. '> |  ||' .. (QBCore.Functions.GetIdentifier(src, 'ip') or 'undefined') .. '|| | ' .. (QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') .. ' | ' .. cData.citizenid .. ' | ' .. src .. ') loaded..')
    end
end)
